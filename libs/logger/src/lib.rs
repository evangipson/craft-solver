//! # [`logger`](crate)
//! The [`logger`](crate) crate contains macros to make it easy to log messages
//! to the console of varying [`LogSeverity`].
//!
//! Make sure to set the logger severity with [`set_logging_severity`] if you'd
//! like to hide or show log messages based on the macros that are called to
//! log any messages.
//!
//! It provides helpful `#[macro_export]` macros to enable logging throughout
//! an application:
...
//! # Examples
//! ## Basic logging
//! [`log_info`](macro@log_info) can be used to log out an informational
//! message:
//! ```rust
//! use logger::log_info;
//!
...
//! [`set_logging_severity`] can be used to filter out all debugging log
//! messages:
//! ```rust
//! use logger::{
//!     self,
...
#[doc = r#"
# log_debug
The [`log_debug`](macro@log_debug) macro logs a message as debugging
information the console, as long as [`set_logging_severity`] has been used to
set the logger's severity to [`LogSeverity::Debug`].
...
#[doc = r#"
# log_info
The [`log_info`](macro@log_info) macro logs an informational message to the
console, as long as [`set_logging_severity`] has been used to set the logger's
severity to [`LogSeverity::Info`] or anything less severe.
...
#[doc = r#"
# log_warning
The [`log_warning`](macro@log_warning) macro logs a warning message to the
console, as long as [`set_logging_severity`] has been used to set the logger's
severity to [`LogSeverity::Warning`] or anything less severe.
...
/// [`log`] will log a message to the console, provided the `severity` is
/// greater than or equal to the severity that has been set with
/// [`set_logging_severity`].
///
/// Using the macros such as [`log_info`](macro@log_info) is preferable and
...
/// [`set_logging_severity`] will set the logger's global severity to the
/// provided `severity`, to filter log messages to only show messages that are
